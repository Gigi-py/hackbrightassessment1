...
def city_output():
    city = input("Enter your hometown here:")
...
if city == "San Francisco":
    ...
    print ("Hi" + " " + full_name + " ,I'd like to visit " + city)
...
# Write your function here
def shipping_cost(base_price,state,tax_rate):
    if state == CA:
        total_price = base_price * (1+tax_rate) * (1+0.03)
    elif state == PA:
        total_price = base_price * (1+tax_rate) + 2
    elif state == MA:
        if base_price < 100:    
            total_price = base_price * (1+tax_rate) + 1
        else:
            total_price = base_price * (1+tax_rate) + 3
    else:
...
# Write your function here
def function1(word): # outer function
